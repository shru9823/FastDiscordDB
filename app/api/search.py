...
# redis = await get_redis()
    # if not keyword:
    #     raise HTTPException(status_code=400, detail="Keyword must not be empty")
    # cache_key = f"exact_search_keyword_search:{keyword}"
    # cached_data = await redis.get(cache_key)
    # if cached_data:
    #     return {"data": cached_data, "source": "cache"}
    # messages = await search_by_keyword(keyword, db)
    # await redis.setex(cache_key, 3600, str(messages))  # Cache for 1 hour
    # if not messages:
    #     raise HTTPException(status_code=404, detail="No messages found")
    # return messages
...
#     if not keyword:
#         raise HTTPException(status_code=400, detail="Keyword must not be empty")
#     cache_key = f"exact_search_keyword_elasticsearch:{keyword}"
#     cached_data = await redis.get(cache_key)
#     if cached_data:
#         return {"data": cached_data, "source": "cache"}
#     messages = await elasticsearch_search_by_keyword(keyword, es)
#     await redis.setex(cache_key, 3600, str(messages))  # Cache for 1 hour
#     if not messages:
#         raise HTTPException(status_code=404, detail="No messages found")
#     return messages
...
# @router.get("/api/es/chats/search/by-date")
# async def search_by_date(start_date: date, end_date: date):
#     redis = await get_redis()
#     try:
#         cache_key = f"elasticsearch_date_range:{start_date}-{end_date}"
#         cached_data = await redis.get(cache_key)
#         if cached_data:
#             return {"data": cached_data, "source": "cache"}
#         messages = await elasticsearch_search_by_date_range(start_date, end_date, es)
#         await redis.setex(cache_key, 3600, str(messages))  # Cache for 1 hour
#     except es_exceptions.NotFoundError:
#         raise HTTPException(status_code=404, detail="No messages found")
#     except Exception as e:
...
#     cache_key = f"exact_search_date_range:{start_date}-{end_date}"
#     cached_data = await redis.get(cache_key)
#     if cached_data:
#         return {"data": cached_data, "source": "cache"}
#     messages = await search_by_date_range(start_date, end_date, db)
#     await redis.setex(cache_key, 3600, str(messages))  # Cache for 1 hour
#     if not messages:
#         raise HTTPException(status_code=404, detail="No messages found")
#     return messages
