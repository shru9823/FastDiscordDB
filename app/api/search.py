...
from ..services.chat_queries import search_by_keyword, exact_search_by_keyword
...
@router.get("/exact_search/keyword/")
# @router.get("/search/keyword/", response_model=List[ChatMessageDisplay])
async def exact_search_keyword(keyword: str, db: db_dependency):
    if not keyword:
        raise HTTPException(status_code=400, detail="Keyword must not be empty")
    messages = exact_search_by_keyword(keyword, db)
    if not messages:
